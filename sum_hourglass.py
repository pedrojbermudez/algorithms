...
class Solution:
  def maxSum(self, grid: List[List[int]]) -> int:
    # Checking if first grid is valid
    if len(grid) < 3 or len(grid) > 150:
      return -1
    ...
    for row in positions:
      if len(grid[row]) < 3 or len(grid[row]) > 150:
        return -1

      for col in positions[row]:
        if grid[row][col] < 0 or grid[row][col] > 106:
          return -1
        total += grid[row][col]
...
def main():
  ...
  print(sol.maxSum([[1,2,3],[4,106,6],[7,8,9]]))
