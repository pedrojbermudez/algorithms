...
class Solution:
  def myAtoi(self, s: str) -> int:
    ...
    n = 0
    empty = True # empty denotes we have not seen any number
    sign = 1
    MAX_VALUE = 2**31-1
    MIN_VALUE = -2**31
    ...
    for c in s:
      if empty:
        if c == ' ': 
          continue # ignoring the leading whitespace
        elif c == '-': 
          sign = -1 # final answer is a negative number
        elif c.isdigit(): 
          n = ord(c) - 48 # the first digit of number
        elif c != '+': 
...
def main():
  ...
  print(sol.myAtoi("  +4193 with words"))
  print(sol.myAtoi("sdfs 4193 with words"))
